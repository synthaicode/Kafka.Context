    ...
    bool EnforceMatch)
{
    public static TopicExpectation ForEntityType(Type entityType, KsqlDslOptions options)
    {
        ...
        var attr = entityType.GetCustomAttribute<KafkaTopicAttribute>(inherit: true)
            ?? throw new InvalidOperationException($"Missing [{nameof(KafkaTopicAttribute)}] on entity type '{entityType.FullName}'.");

        var topicName = attr.Name;
        return Build(topicName, attr.PartitionCount, attr.ReplicationFactor, options);
    }
