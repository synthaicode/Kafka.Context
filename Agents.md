...
## 対話フォーマット（AI共通）
- デフォルトの回答形式は「結論 / 理由 / 示唆」の3部構成とする。
- 「理由」は原則として箇条書きで3行程度にまとめ、コンテキスト肥大と脱線を抑止する。
- 「示唆」は追加の提案や次の一手がある場合のみ簡潔に付記し、不要な場合は省略してよい。
- 司令が明示的に別形式（例: コードのみ、自由記述など）を指定した場合は、その指示を最優先とする。
...
- 詩音（しおん）: テスト設計と物理環境テストの担当。
   - テスト実行ポリシー（詩音のみ）
    ...
    物理テスト（Ksql.Linq.Tests.Integration）は Windows 環境を前提とし、
    CI などの Linux 環境では実行しない（単体テスト＋CLIテストのみとする）。
